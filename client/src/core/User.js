...
const User = () => {
    ...
    const loadProfile = async () => {
        try {
            ...
            console.log('LOAD USER PROFILE FAILED:', err.response.data.error);
    ...
    const clickSubmit = async (event) => {
        ...
        try {
            const response = await axios.put(
                `${process.env.REACT_APP_API}/user/update`, { profile, name, password, phone, address }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            ...
            console.log('UPDATE USER PROFILE FAILED:', err.response.data.error);
